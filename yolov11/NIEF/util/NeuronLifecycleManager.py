...
class NeuronLifecycleManager:
    def __init__(self, model, plasticity_factor=0.3, protection_factor=0.7,
                 ...
                 plasticity_decay=0.95, update_interval=5, freeze_protected=False):
        """
        动态神经元生命周期管理器
    ...
    def _decide_neuron_actions(self, activation_metrics, contribution_metrics):
        ...
        for name, activation in activation_metrics.items():
            ...
            for i in range(n_neurons):
                ...
                # if activation_norm[i] < effective_protect_threshold or contribution_norm[i] < effective_reset_threshold:  # 平衡条件
                #     reset_candidates.append((name, i))



                # if contribution_norm[i] < effective_reset_threshold:
                #     reset_candidates.append((name, i))

                # if activation_norm[i] < effective_protect_threshold:
                #     reset_candidates.append((name, i))
