...
class GnT(object):
    ...
    def __init__(
        ...
        """
        self.util = [torch.zeros(self.net[i * 2].out_channels).to(self.device) for i in range(self.num_hidden_layers)]
        self.bias_corrected_util = \
            [torch.zeros(self.net[i * 2].out_channels).to(self.device) for i in range(self.num_hidden_layers)]
        self.ages = [torch.zeros(self.net[i * 2].out_channels).to(self.device) for i in range(self.num_hidden_layers)]
        self.m = torch.nn.Softmax(dim=1)
        self.mean_feature_act = [torch.zeros(self.net[i * 2].out_channels).to(self.device) for i in range(self.num_hidden_layers)]
    ...
    def compute_bounds(self, hidden_activation, init='kaiming'):
        ...
        if init == 'default':
            bounds = [sqrt(1 / self.net[i * 2].in_channels) for i in range(self.num_hidden_layers)]
        elif init == 'xavier':
            bounds = [torch.nn.init.calculate_gain(nonlinearity=hidden_activation) *
                      sqrt(6 / (self.net[i * 2].in_channels + self.net[i * 2].out_channels)) for i in
                      range(self.num_hidden_layers)]
        elif init == 'lecun':
            bounds = [sqrt(3 / self.net[i * 2].in_channels) for i in range(self.num_hidden_layers)]
        else:
            bounds = [torch.nn.init.calculate_gain(nonlinearity=hidden_activation) *
                      sqrt(3 / self.net[i * 2].in_channels) for i in range(self.num_hidden_layers)]
        bounds.append(1 * sqrt(3 / self.net[self.num_hidden_layers * 2].in_channels))
        return bounds
        ...
        with torch.no_grad():
            for i in range(self.num_hidden_layers):
                ...
                current_layer.weight.data[features_to_replace[i], :] += \
                    torch.empty(num_features_to_replace[i], current_layer.in_channels).uniform_(
