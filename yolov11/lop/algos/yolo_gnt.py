from torch.nn import Conv2d, Linear, BatchNorm2d, Upsample
...
class YoloGnT(object):
    ...
    def __init__(self, net, hidden_activation, decay_rate=0.99, replacement_rate=1e-4, util_type='weight',
                 maturity_threshold=1000, device=torch.device("cuda")):
        super(YoloGnT, self).__init__()
        self.max_up_sample = 3
        self.net = net
        self.bn_layers = []
        self.weight_layers = []
    ...
    def get_weight_layers(self, nn_module: torch.nn.Module):
        if isinstance(nn_module, Upsample): return
