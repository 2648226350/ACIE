...
class SSRE(BaseLearner):
    ...
    def _compute_ssre_loss(self,inputs, targets):
        if self._cur_task == 0:
            ...
            loss_clf = self.loss(logits/self.args["temp"], targets)
        ...
        with torch.no_grad():
            ...
            mask = weights
        logits = self._network(inputs)["logits"]
        loss_clf = self.loss(logits/self.args["temp"],targets,reduction="none")
        ...
        loss_proto = self.args["lambda_proto"] * self.loss(proto_logits/self.args["temp"], proto_targets)
